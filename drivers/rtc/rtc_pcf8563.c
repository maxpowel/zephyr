 ...
 */
int pcf8563_set_time(const struct device *dev, const struct rtc_time *time)
{
	...
	raw_time[0] = bin2bcd(time->tm_sec);

	/* Set minutes */
	raw_time[1] = bin2bcd(time->tm_min);

	/* Set hours */
	raw_time[2] = bin2bcd(time->tm_hour);

	/* Set days */
	raw_time[3] = bin2bcd(time->tm_mday);

	/* Set weekdays */
	raw_time[4] = time->tm_wday;

	/*Set month */
	raw_time[5] = bin2bcd(time->tm_mon);

	/* Set year */
	raw_time[6] = bin2bcd(time->tm_year);
...
int pcf8563_get_time(const struct device *dev, struct rtc_time *time)
{
	const struct pcf8563_config *config = dev->config;
	int ret = 0;
	uint8_t raw_time[7] = {0};
	 ...
	 * for other stuff
	 */
	time->tm_sec = bcd2bin(raw_time[0] & GENMASK(6,0));

	/* Get minutes */
	time->tm_min = bcd2bin(raw_time[1] & GENMASK(6,0));

	/* Get hours */
	time->tm_hour = bcd2bin(raw_time[2] & GENMASK(5,0));

	/* Get days */
	time->tm_mday = bcd2bin(raw_time[3] & GENMASK(5,0));

	/* Get weekdays */
	time->tm_wday = raw_time[4] & GENMASK(2,0);

	/* Get month */
	time->tm_mon = bcd2bin(raw_time[5] & GENMASK(4,0));

	/* Get year */
	time->tm_year = bcd2bin(raw_time[6]);
				  ...
				  const struct rtc_time *timeptr)
{
	...
	if ((mask & RTC_ALARM_TIME_MASK_MINUTE) != 0) {
		regs[0] = bin2bcd(timeptr->tm_min) & GENMASK(6,0);
	} else {
	...
	if ((mask & RTC_ALARM_TIME_MASK_HOUR) != 0) {
		regs[1] = bin2bcd(timeptr->tm_hour) & GENMASK(5,0);
	} else {
		regs[1] = BIT(7);
	}

	if ((mask & RTC_ALARM_TIME_MASK_MONTHDAY) != 0) {
		regs[2] = bin2bcd(timeptr->tm_mday) & GENMASK(5,0);
	} else {
		regs[2] = BIT(7);
	}

	if ((mask & RTC_ALARM_TIME_MASK_WEEKDAY) != 0) {
		regs[3] = bin2bcd(timeptr->tm_wday) & GENMASK(2,0);
	} else {
		regs[3] = BIT(7);
	}
	...
	i2c_reg_write_byte_dt(&config->i2c, PCF8563_CONTROL2_REGISTER, GENMASK(1,0));
				  ...
				  struct rtc_time *timeptr)
{
	...
	if (err)
	{
		LOG_ERR("Eror when getting alarm time: %i", err);
		return err;
	}
...
static int pcf8563_alarm_is_pending(const struct device *dev, uint16_t id)
{
	...
	if (err)
	{
		LOG_ERR("Eror when getting the control register 2: %i", err);
		return err;
	}
				      ...
				      rtc_alarm_callback callback, void *user_data)
{
	...
	int ret = gpio_pin_configure_dt(&config->int1, GPIO_INPUT | GPIO_PULL_UP);
	if (ret < 0) {
...
int pcf8563_init(const struct device *dev)
{
	...
	/* Check if it's alive. */
	uint8_t reg;
	int ret = i2c_reg_read_byte_dt(&config->i2c, PCF8563_CONTROL1_REGISTER, &reg);
	if (ret)
	{
